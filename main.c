...
#define TRUE 1
#define FALSE 0
#define PATH "file.wav"

// WAVE header structure

#define A0  -1.8349094955664635
#define A1	0.8475346360324341
#define B0	1.4491094061255398
#define B1	-1.6416010675393176
#define B2	0.5917336579340403

unsigned char buffer4[4];
unsigned char buffer2[2];

char* seconds_to_time2(float seconds);
FILE *ptr,*out;
char *filename;
...
int main(int argc, char **argv) {
	...
	int i;
	float* samples_float;
	filter_t* my_filter = filter_create(A0,A1,B0,B1,B2,16);

	AUDIO_FILE* audio = new_audio_file("pene.wav");
	read_pcm_file(audio);
	samples_float = (float*)malloc(sizeof(samples_float)*audio->samples->num_samples);
	uint_2_float(*audio->samples->channels, audio->samples->num_samples,samples_float,16);

	f_samples = fopen("filt.txt","w");
	for(i = 0; i < audio->samples->num_samples; i++){
		fprintf(f_samples,"%f\n",*(samples_float+i));
	}
	fclose(f_samples);
	tst = fopen("tst_filt.txt","w");

	filter_samples(samples_float,audio->samples->num_samples,my_filter);

	f_samples = fopen("filt_out.txt","w");
	for(i = 0; i < audio->samples->num_samples; i++){
		fprintf(f_samples,"%f\n",*(samples_float+i));
	}
	fclose(f_samples);
	fclose(tst);
	delete_pcm_file(audio);
	free(my_filter);
	free(samples_float);
//////
...
//	// open file
//	printf("Opening  file..\n");
//	ptr = fopen("pene.wav", "rb");
//	out = fopen("test2.txt", "wb");
//	if (ptr == NULL) {
...
//			seconds_to_time2(duration_in_seconds));
//
...
//								data_in_channel = data_buffer[0];
//							}
//							if(data_in_channel > 32767){
//								data_in_channel -= 65536;
//							}
//
 ...
 **/
char* seconds_to_time2(float raw_seconds) {
	char *hms;
